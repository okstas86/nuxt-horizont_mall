	...
	>
		<NuxtLink :to="`/item/${product.id}`">
			...
			<div id="ProductDetails">
				<span class="flex item-center justify-start gap-3 px-1 pt-1">
					<span class="text-[#8cfc2f] font-semibold">${{ priceComputed }}</span>
					<span class="text-gray-700 text-sm text-light line-through"
						>${{ oldPriceComputed }}</span
					>
				</span>
				<span
					class="px-1 relative -top-1.5 text-[#8cfc2f] text-xs font-senibold"
					>Extra 5% off</span
				>
				<div class="flex items-center gap-1 px-1 relative -top-1">
					<span
						class="bg-[#8cfc2f] text-white text-[9px] font-semibold px-1.5 rounded-sm"
						>Welcome Deal
					</span>
					<span
						class="bg-[#f5f5f5] border text-gray-700 text-[9px] font-semibold px-1.5 rounded-sm"
						>Welcome Deal
					</span>
				</div>
